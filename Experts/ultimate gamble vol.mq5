...
#include <gride-martingale[1].mqh>
...
//---
   ...
   newBar.SetPeriod(tf);
   gmExpert.doInit(label,bbPeriod,bbDeviation,bbDeviation,atrPeriod,
                   kamaPeriod,fastEndPeriod,slowEndPeriod,smoothPower,
                   filter,filterPeriod,filterDifference,price,
   ...
   martingale.doInit(stoTf,k,d,s,maTf,inPeriod,maPeriod,fastMaPeriod,slowMaPeriod,cmoPeriod,apPrice);
   ...
   if(newBar.isNewBar()>0)
     {
      ...
      if(isBuy)
      {  
         //trade.Buy(0.2,NULL,SymbolInfoDouble(NULL,SYMBOL_ASK),0,0,NULL);
         //trade.BuyStop(0.2,SymbolInfoDouble(NULL,SYMBOL_ASK)+2000,NULL,0,0,ORDER_TIME_GTC,0,NULL);
      ...
      if(isSell)
      {
         ...
         //bool internalSell = gmExpert.checkIntSell();
         //Check the direction of open trades
         //bool opTrades = OpenTrades(); wait till I know what i want to do with you
         
         bool internalSell = gmExpert.checkIntSell();
         bool internalClose = gmExpert.checkIntSellOpp();
         //
         bool thisIsFirst = prevSell;
         prevSell = true;
         // open grids 
         if (thisIsFirst)
         {
            openGrids("sell",maxTrades);
         }
         if(internalSell)
         {
            openGrids("sell",opOrds);
         }
   ...
   if(ordType == "buy")
      {  
        ...
        if (maxt == 1)
        {
           ...
           //trade.Buy(0.01,NULL,Ask,Ask-30,Ask+30,NULL);
           //trade.Buy(0.001,NULL,Ask,0,0,NULL);
        }
        ...
        else if (maxt > 1 )
        {
         for(int i= 1; i< maxt+1; i++)
           ...
           trade.BuyStop(0.001,Ask+(1000*i),_Symbol,Ask-(1000*i)-3000,Ask+(1000*i)+3000,ORDER_TIME_GTC,0,NULL);
           ///trade.BuyStop(0.001,Ask+(atrval*i),_Symbol,0,0,ORDER_TIME_GTC,0,NULL);
           }
       ...
       double tp = Bid - 100;
        if (maxt == 1)
        {
            ...
            trade.Sell(0.001,NULL,Bid,Bid+4000,Bid-3000,NULL );
        }
        else if (maxt>1)
        
        {
           ...
           {
            ///trade.SellStop(0.001,Bid-(1000*i),_Symbol,Bid-(1000*i)+3000,Bid-(1000*i)-3000,ORDER_TIME_GTC,0,NULL);
            trade.SellStop(0.001,Bid-(1000*i),_Symbol,Bid+(1000*i)+3000,Bid-(1000*i)-3000,ORDER_TIME_GTC,0,NULL);
           } 
        }
